...
export async function POST(request: NextRequest) {
  try {
    ...
    const { markdownText, roomId, sessionId, compileType } = body;
    ...
    // 2. Premium Compile Flow (Auth & Payment Gated)
    ...
    const compiledPack = await compilePromptPack(markdownText, techSignatures, false);
