...
## Phase 36: Unified AI Compilation & Gating (Completed)

- [x] **Step 36.1:** Remove the `basic` compile type from `src/app/api/compile/route.ts` and refactor the main compiler entry `src/lib/prompt-compiler/index.ts` to deprecate `localMockCompile`.
- [x] **Step 36.2:** Simplify the Room page top toolbar to display a single "COMPILE WITH AI" action button instead of the split compile mode dropdown.
- [x] **Step 36.3:** Implement a custom `compile-setup-modal.tsx` component that allows users to configure a free BYOK key or redirect to Stripe credits checkout when they request compilations without configurations.
- [x] **Step 36.4:** Update held hosted LLM brand mentions across `settings-modal.tsx`, `pricing-client.tsx`, and `faq-client.tsx` to Claude Sonnet 4.6.
